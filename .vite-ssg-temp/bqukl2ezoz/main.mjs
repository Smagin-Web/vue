...
async function includedRoutes(paths, routes2) {
  const apiClient = new MyApiClient({}.MY_API_KEY);
  return Promise.all(
    routes2.flatMap(async (route) => {
      return route.name === "Blog" ? (await apiClient.fetchBlogSlugs()).map((slug) => `/blog/${slug}`) : route.path;
    })
  );
}
export {
